...
We assume that Geant4 toolkit is installed in your system (this may change in the future to install Geant4 libraries as a Julia binary package). In addition you need to have `wrapit` install, which itself requires `libclang` to be installed. 

- what C++ classes get wrapped is controlled by the file `gen/Geant4.wit`. See the documentation of WrapIt for the details. 
- run the `gen/generate.jl` script generate the wrapper code and build the library. You may need to adapt the `LCG_prefix` to the location of your local installation of Geant4.
- the resulting library should be located in `gen/build/lib/libGeant4Wrap`

## Getting started
Import the `Geant4` module. All the wrapped Geant4 classes are exported since they are prefixed by `G4` minimizing any name clash with other Julia symbols. 
```julia
...
CxxWrap.CxxWrapCore.ConstCxxRef{G4String}(Ptr{G4String} @0x00007f9fc763b1e8)
...
For the time being there is only one example: basic/B1. To run it execute `julia --project=. examples/B1.j` or execute the notebook `B1.ipynb`
